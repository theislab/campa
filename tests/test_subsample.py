...
class TestSubset:
    ...
    def test_random_frac_nonemptyRes(self, frac, num_objects):
        ...
        assert math.isclose(
            len(mpp_data_sub.mpp) / len(mpp_data.mpp), frac, rel_tol=0.05
    ...
    def test_random_frac_per_obj_nonemptyRes(self, frac_per_object):
        ...
        for obj_id in mpp_data_sub.unique_obj_ids:
            assert math.isclose(
                len(mpp_data_sub.subset(obj_ids=[obj_id], copy=True).mpp)
                / len(mpp_data.subset(obj_ids=[obj_id], copy=True).mpp),
                frac_per_object,
                rel_tol=0.05,
                abs_tol=0.05,
            ), f"test failed for frac {frac_per_object}"
