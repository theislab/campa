...
@pytest.fixture
def test_experiment_full_data_clustered():
    model_name = id_generator(size=6)
...
def test_extract_features(test_experiment_full_data_clustered):
    ...
    for data_dir in ["184A1_unperturbed/I09", "184A1_meayamycin/I12"]:
        ...
        test_extr = FeatureExtractor.from_adata(os.path.join(EXPERIMENT_DIR, test_experiment, 'aggregated/full_data', data_dir, 'features.h5ad'))
        reference_extr = FeatureExtractor.from_adata(os.path.join(EXPERIMENT_DIR, 'reference_experiment/cVAE/aggregated/full_data', data_dir, 'features.h5ad'))
        assert test_extr.compare(reference_extr)[0]
        

def test_plot_intensity_features():
