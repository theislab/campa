...
def extract_features(params):
    ...
    for data_dir in data_dirs:
        ...
        if os.path.exists(adata_fname):
            ...
            extr = FeatureExtractor(exp, data_dir=data_dir, cluster_name=params['cluster_name'], 
                cluster_dir=params['cluster_dir'], cluster_col=params['cluster_col'])
        # extract features
        ...
        if 'co-occurrence' in params["features"]:
            ...
            if co_occ_params['logspace']:
                interval = np.logspace(np.log2(co_occ_params['min']),np.log2(co_occ_params['max']),co_occ_params['nsteps'], base=2).astype(np.float32)
            else:
                interval = np.linspace(co_occ_params['min'],co_occ_params['max'],co_occ_params['nsteps']).astype(np.float32)
        ...
        if 'object-stats' in params['features']:
            ...
            extr.extract_object_stats(features=obj_params['features'], intensity_channels=obj_params['channels'])
