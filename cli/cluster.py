...
from campa.tl import Cluster, Experiment, Predictor
...
def prepare_full_dataset(args):
    ...
    for data_dir in exp.data_params["data_dirs"]:
        ...
        if exp.config["cluster"]["cluster_rep"] == "mpp":
            # just save mpp
            mpp_data.write(
                os.path.join(exp.full_path, args.save_dir, data_dir),
            ...
            pred.predict(
                ...
                save_dir=os.path.join(exp.full_path, args.save_dir, data_dir),
                mpp_params=mpp_params,
            )
...
def create_cluster_data(args):
    ...
    cluster_config = {
        ...
        "subsample_kwargs": {"frac": args.frac},
    }
    save_dir = (
        args.save_dir if args.save_dir is not None else f"aggregated/sub-{args.frac}"
    )
...
def project_data(args):
    exp = Experiment.from_dir(args.experiment_dir)
    # set up cluster data
    cl = Cluster.from_cluster_data_dir(
        os.path.join(exp.dir, exp.name, args.cluster_data_dir)
    )
    cl.set_cluster_name(args.cluster_name)
    assert (
        cl.cluster_mpp.data(args.cluster_name) is not None
    ), f"cluster data needs to contain clustering {args.cluster_name}"
    # iterate over all data dirs
    data_dirs = (
        exp.data_params["data_dirs"] if args.data_dir is None else [args.data_dir]
    )
    for data_dir in data_dirs:
        # load mpp_data with cluster_rep
        mpp_data = MPPData.from_data_dir(
            data_dir,
            base_dir=os.path.join(exp.full_path, args.save_dir),
            keys=["x", "y", "obj_ids", cl.config["cluster_rep"]],
        )
        cl.project_clustering(
            mpp_data, save_dir=os.path.join(exp.full_path, args.save_dir, data_dir)
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "experiment_dir", metavar="experiment-dir", help="relative to EXPERIMENT_PATH"
    )
    ...
    prepare = subparsers.add_parser(
        "prepare-full", help="Prepare full data for clustering. Predicts cluster-rep."
    )
    ...
    project.add_argument(
        "--cluster-name", default="clustering", help="name of clustering to project"
    )
