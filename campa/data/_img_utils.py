...
class BoundingBox(object):
    ...
    def __str__(self):
        return 'BoundingBox at ({},{}) with shape ({},{})'.format(self.ymin, self.xmin, self.h, self.w)
    
    def __repr__(self):
        return 'BoundingBox({},{},{},{})'.format(self.ymin, self.xmin, self.h, self.w)

    def as_int(self):
        return BoundingBox(int(round(self.ymin)), int(round(self.xmin)), int(round(self.h)), int(round(self.w)))
    ...
    def fit_in_shape(self, shape):
        ...
        fitted_bbox = BoundingBox.from_corners([max(0,self.ymin), max(0,self.xmin), min(self.ymax,ah), min(self.xmax,aw)])
    ...
    def crop(self, arr):
        ...
        res[yoff1:bbox.h-yoff2,xoff1:bbox.w-xoff2] = arr[bbox_crop.ymin:bbox_crop.ymax, bbox_crop.xmin:bbox_crop.xmax]
        return res