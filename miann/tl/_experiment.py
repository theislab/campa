...
class Experiment:
    ...
    def __init__(self, config):
        ...
        # load data_params
        self.data_params = json.load(open(os.path.join(data_config.DATASET_DIR, self.config['data']['dataset_name'], 'params.json'), 'r'))
        # create exp_path
    ...
    def get_split_cluster_annotation(self, cluster_name='clustering'):
        ...
        """
        fname = os.path.join(self.full_path, f'results_epoch{self.epoch:03d}', self.config['evaluation']['split'], f"{cluster_name}_annotation.csv")
